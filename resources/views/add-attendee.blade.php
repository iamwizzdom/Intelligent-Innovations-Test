        ...
        $('#add-talk-form').submit(function (e) {
            ...
            mInstance.addToRequestQueue({
                ...
                success: (data, status, xhr) => {
                    ...
                    if (data.status !== true) {
                        errorMsg('Attendee Failed', typeof data.message !== 'string' ? serializeMessage(data.message) : data.message, 'Ok');
                        return false;
                    }

                    successMsg('Attendee Successful', data.message);
                    self[0].reset();

                    setTimeout(() => {
                        window.location.href = "{{ route('attendees') }}";
                    }, 2000);

                },
                ontimeout: () => {
                    swal.hideLoading();
                    errorMsg('Attendee Failed', 'Request failed, as it timed out', 'Ok');
                },
